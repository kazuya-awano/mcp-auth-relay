...
MCP Auth Relay は、Dify からリモート MCP サーバーを使うためのプラグインです。2つのツールと2つのエンドポイントを提供し、OAuth トークンをユーザー単位で保存しながら、MCP ツールを Dify から安定して呼び出せる形に変換します。
...
7. Agent または Workflow に `mcp_tool_list` と `mcp_tool_call` の両方を追加します。
...
Agent または Workflow に次の2つを追加します。

- `mcp_tool_list`
- `mcp_tool_call`

`mcp_tool_list` は利用可能な MCP ツールと認証 URL の取得に使い、`mcp_tool_call` は選択した `tool_ref` を実行するために使います。

## 利用フロー

1. `mcp_tool_list` を呼び出します。
2. 対象サーバーが分かっている場合は `server_id` または `server_ids` を指定します。
3. レスポンスに `auth_required` が含まれる場合は `login_url` を開いて認証します。
4. 必要なら `mcp_tool_list` を再実行します。
